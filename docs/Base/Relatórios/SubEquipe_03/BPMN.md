# SubEquipe_03 — BPMN

## Descrição

Modelagem de processo de negócio da SubEquipe_03, no escopo do **FOCO_02 — Engenharia Reversa & Modelagem BPMN**. Representa, na notação BPMN, um fluxo do software levantado por Engenharia Reversa.

## Objetivo

Modelar em BPMN um fluxo identificado durante o processo de Engenharia Reversa, evidenciando as atividades, os eventos e os pontos de decisão do processo.

## Metodologia

*Descrever o contexto no qual foi aplicada a Engenharia Reversa, o processo seguido, as etapas percorridas, as ferramentas utilizadas e os achados. Embasar na literatura as colocações.*

## Conteúdo

### Processo de Engenharia Reversa Aplicado

*Descrever o processo aplicado e os achados.*

### Modelagem BPMN

*Inserir aqui o modelo BPMN do fluxo levantado, com legenda e fonte.*

<p align="center">Figura 1: Modelo BPMN do fluxo de &lt;&lt;nome do fluxo&gt;&gt;. Fonte: Autores, 2026.</p>

## Referências

*Listar as referências utilizadas, em ordem alfabética.*

## Nível de Contribuição dos Integrantes

| Nome | % de Contribuição |
|------|-------------------|
|      |                   |

<p align="center">Tabela 1: Contribuição dos integrantes. Fonte: Autores, 2026.</p>

## Histórico de Versão

| Versão | Data | Descrição | Autor(es) | Revisor |
|:------:|------|:----------|:----------|:--------|
| 1.0 | 22/08/2026 | Criação da página no modelo do artefato padrão | Marcelo de Araújo Lopes | |

<p align="center">Tabela 2: Histórico de versão. Fonte: Autores, 2026.</p>

Ver também: [Artefato Generalista](ArtefatoGeneralista.md) · [Rich Picture](RichPicture.md) · [NFR Framework](NFRFramework.md) · [IA Generativa](IAGenerativa.md)
